...
namespace Greyman{
	public class OffScreenIndicatorManagerCanvas : OffScreenIndicatorManager {
		...
		protected override void UpdateIndicatorPosition(ArrowIndicator arrowIndicator, int id = 0){
			Vector3 v2DPos = camera.WorldToScreenPoint(arrowIndicator.target.localPosition + arrowIndicator.indicator.targetOffset);
			float angle;
			bool behindCamera;

			Vector3 heading = arrowIndicator.target.position - camera.transform.position;
			behindCamera = (Vector3.Dot(camera.transform.forward, heading) < 0);
